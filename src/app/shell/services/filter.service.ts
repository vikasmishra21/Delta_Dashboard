...
export class FilterService {
  ...
  filterSyntax: any = new Object({
    Key: '',
    Value: ''
  });
  filtercounter = 0;
  typeFilter: Map<string, FilterConfig>;

  returnTypeFilterArray: Array<any> = new Array<any>();
  ...
  readonly selectedChoices: Map<string, Array<FilterOption>>;

  constructor(private communicationService: CommunicationService) {
    this.filterConfigurations = new Map<string, FilterConfig>();
    this.typeFilter = new Map<string, FilterConfig>();
  ...
  addFilterConfig(filter: FilterConfig) {
    ...
    if (filter.actAs == 1 || filter.actAs == 3) {
      this.typeFilter.set(filter.variable, filter);
    }
  }
  ...
  removeChoice(variable: string, choice: FilterOption) {
    ...
    if (variable === 'Quarterly' || variable === 'Yearly' || variable === 'Semiannual') {
  ...
  /**
   * To Concatinate All Filter by type;
   */
  concatinateDataForApi(config: FilterConfig) {
    let callApi = true;
    this.filtercounter++
    if (this.filtercounter < this.typeFilter.size && config.actAs != 2) {
      if (this.filtercounter == 1) {
        this.filterSyntax.Key = 'side{' + config.variable + '}';
      } else {
        this.filterSyntax.Key = this.filterSyntax.Key + '+' + 'side{' + config.variable + '}';
      }
      callApi = false;
    }
    return callApi;
  ...
  private getDataFromAnalysis(config: FilterConfig): Observable<FilterCascadeOutput> {
    let filterSyntax2 = new Object({
      ...
      if (config.actAs != 2) {
        this.concatinateDataForApi(config);
        let returnTypeFilter: Subject<FilterCascadeOutput> = new Subject<FilterCascadeOutput>();
        let requestInfo = { variableName: config.variable, observable: returnTypeFilter };
        this.returnTypeFilterArray.push(requestInfo);
        
        if(this.filtercounter == this.typeFilter.size-1){
         // if (config.actAs != 2) {
            filterSyntax2 = new Object({
              Key: this.filterSyntax.Key ,
              Value: ''
            });
        //  }
        this.filtercounter = 0;
          return this.communicationService.getFilterData(filterSyntax2)
            .pipe(map(d => {
              let dataArray = [];
              const obj = {
                options: [],
              ...
              this.returnTypeFilterArray.forEach((val,i)=>{
                obj.options = [];
                obj.variableText = d[val.variableName].text;
                Object.keys(d[val.variableName].options || {}).map((code, index) => {
                  obj.options[d[val.variableName].options[code].sequance] = d[val.variableName].options[code];
                });
                obj.options = obj.options.filter(item => {
                  return typeof item === 'object';
                });
                FilterCaching.cacheData(d[val.variableName].code, obj, CacheTypes.InMemory);
                val.observable.next(obj);
              });
              //let newOnject: FilterCascadeOutput = new FilterCascadeOutput();
              //Object.assign(newOnject, dataArray)
              return obj;
            }));
        }else{
          return returnTypeFilter;
        }
      }else{
        return this.communicationService.getFilterData(filterSyntax2)
            .pipe(map(d => {
              ...
              FilterCaching.cacheData(d[config.variable].code, obj, CacheTypes.InMemory);
              return obj;
            }));
      }
