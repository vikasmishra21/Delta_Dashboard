export class DataTable {
  ...
  previousScoreRows: Map<string, Array<number>>;
  headers: Array<string>;
  comparisonRows: Map<string, Array<number>>;
  significance: Map<string, Array<number>>;
  bases: Map<string, Array<number>>;
  comparisonBases: Map<string, Array<number>>;
  /**
   * All Method are useto Add Data into Datatable
   */
  constructor() {
    this.rows = new Map<string, Array<number>>();
    this.previousScoreRows = new Map<string, Array<number>>();
    ...
    this.comparisonBases = new Map<string, Array<number>>();
  }
  ...
  setPrivousData(rowName: string, item: number) {
    if (!this.previousScoreRows.has(rowName)) {
      this.previousScoreRows.set(rowName, new Array<number>());
    }
    this.previousScoreRows.get(rowName).push(item);
  }
  ...
  setComparisonBaseRow(rowName: string, item: number) {
    if (!this.comparisonBases.has(rowName)) {
      this.comparisonBases.set(rowName, new Array<number>());
    }
    this.comparisonBases.get(rowName).push(item);
  }
}
