...
export class Break {
  ...
  private DisplaySigTestOnBothValues: boolean;
  ...
  constructor(variableMap: VariableMap, measure: number) {
    ...
    this.DisplaySigTestOnBothValues = false;
  }
  ...
  showOptionsSideBreak(options: number[]) {
    if (options.length > 0) {
      this.Command.Show = options;
      this.Command.Order = options;
      if(TimePeriod.PreviousPeriod){
        this.Command.Test = options;
        this.DisplaySigTestOnBothValues = true;
      }
    }
  }
  showOptionsTopBreak(options: number[]) {
    if (options.length > 0) {
      this.Command.Show = options;
      this.Command.Order = options;
    }
