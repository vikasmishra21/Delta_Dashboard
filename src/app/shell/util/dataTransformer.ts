...
export class DataTransformer {
  /**
   * get Data into output form and Converted and added into datatable
   */
  static convertToDataTable(tableName: string, output: CollectionOutput = new CollectionOutput(), rowField?: string,
    ...
    const flatPreviousData: Array<TableOutput> = output.TableOutputPrevious.get(tableName);
    const baseData: Array<TableOutput> = output.Bases.get(tableName);
    const compareBaseData: Array<TableOutput> = output.previousBases.get(tableName);
    ...
    flatData.forEach(data => {
      ...
      dataTable.setPrivousData(data[rowName], data.previousScore);
    });
    baseData.forEach(data => {
      dataTable.setBaseRow(data[rowName], data.Score);
    });
    if(compareBaseData != undefined){
      compareBaseData.forEach(data => {
        dataTable.setComparisonBaseRow(data[rowName], data.Score);
      });
    }
    return dataTable;
  }
