...
export class ChartContainerComponent implements OnInit, OnDestroy {
  ...
  /**
   * Method to convert data in diffrent tables
   * Use perivious and current data and base , added into Output
   * @param output data in form for Output:Datatable from url
   */
  private processOutput(output: CollectionOutput) {
    ...
    if (comparison.isEnabled) {
      ...
      previousOutput.previousBases.set(this.chart.Name, output.getFilteredBases(this.chart.Name,
        TimePeriod.Variable, TimePeriod.PreviousPeriod));
      ...
      output.TableOutputPrevious.set(this.chart.Name, previousOutput.TableOutput.get(this.chart.Name));
      ...
      output.previousBases.set(this.chart.Name, previousOutput.previousBases.get(this.chart.Name));
